...
func (a *AdminHandle) Add(msg *tgbotapi.Message) (err error) {
	...
	nameTariff := fmt.Sprintf("Тариф %v был успешно добавлен!", data.Title)
	h.MsgForUser(*a.Bot, msg.Chat.ID, nameTariff)
	return nil
}
...
func (t *AdminHandle) Del(msg *tgbotapi.Message) (err error) {
	...
	if err != nil {
		h.MsgForUser(*t.Bot, msg.From.ID, "Невозможно удалить тариф.\n\nВозможно введен неверный тариф или такого ID не существует")
		return err
	}
